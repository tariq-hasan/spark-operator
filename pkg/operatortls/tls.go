/*
Copyright 2026 The Kubeflow authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    https://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package operatortls

import (
	"crypto/tls"
	"fmt"
	"strings"
)

// SetupResult holds the TLS options produced by SetupTLS.
type SetupResult struct {
	TLSOpts []func(*tls.Config)
}

// SetupTLS parses the TLS flags and returns TLS options for webhook and metrics servers.
func SetupTLS(minVersion string, cipherSuites []string, enableHTTP2 bool) (SetupResult, error) {
	var result SetupResult

	ver, err := ParseTLSVersion(minVersion)
	if err != nil {
		return result, fmt.Errorf("invalid --tls-min-version %q: %w", minVersion, err)
	}
	result.TLSOpts = append(result.TLSOpts, func(c *tls.Config) {
		c.MinVersion = ver
	})

	if len(cipherSuites) > 0 {
		cipherIDs, err := ParseCipherSuites(cipherSuites)
		if err != nil {
			return result, fmt.Errorf("invalid --tls-cipher-suites: %w", err)
		}
		result.TLSOpts = append(result.TLSOpts, func(c *tls.Config) {
			c.CipherSuites = cipherIDs
		})
	}

	if enableHTTP2 {
		result.TLSOpts = append(result.TLSOpts, func(c *tls.Config) {
			c.NextProtos = []string{"h2", "http/1.1"}
		})
	} else {
		result.TLSOpts = append(result.TLSOpts, func(c *tls.Config) {
			c.NextProtos = []string{"http/1.1"}
		})
	}

	return result, nil
}

// ParseTLSVersion converts a version string to a tls.Version constant.
func ParseTLSVersion(version string) (uint16, error) {
	switch version {
	case "VersionTLS12":
		return tls.VersionTLS12, nil
	case "VersionTLS13":
		return tls.VersionTLS13, nil
	default:
		return 0, fmt.Errorf("unsupported TLS version: %s (use VersionTLS12 or VersionTLS13)", version)
	}
}

// ParseCipherSuites converts cipher suite names to their IDs.
// Empty strings are silently filtered out.
func ParseCipherSuites(names []string) ([]uint16, error) {
	allCiphers := make(map[string]uint16)
	for _, cs := range tls.CipherSuites() {
		allCiphers[cs.Name] = cs.ID
	}
	var ids []uint16
	for _, name := range names {
		name = strings.TrimSpace(name)
		if name == "" {
			continue
		}
		id, ok := allCiphers[name]
		if !ok {
			return nil, fmt.Errorf("unknown cipher suite: %s", name)
		}
		ids = append(ids, id)
	}
	return ids, nil
}
