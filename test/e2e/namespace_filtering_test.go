...
var _ = Describe("Namespace Filtering", func() {
	...
	Context("With namespace selector", func() {
		...
		// It("should process SparkApp when namespace label is added dynamically", func() {
		// 	// Create namespace without label
		// 	nsDynamic, err := createTestNamespaceWithSA(
		// 		ctx,
		// 		fmt.Sprintf("test-dynamic-%s", testID),
		// 		nil, // No labels initially
		// 	)
		// 	Expect(err).NotTo(HaveOccurred())
		// 	defer func() {
		// 		_ = k8sClient.Delete(ctx, nsDynamic)
		// 	}()

		// 	app, err := loadSparkPiExample(
		// 		nsDynamic.Name,
		// 		fmt.Sprintf("test-dynamic-app-%s", testID),
		// 	)
		// 	Expect(err).NotTo(HaveOccurred())
		...
		// 	By("Verifying app stays in New state initially")
		// 	key := types.NamespacedName{Namespace: app.Namespace, Name: app.Name}
		// 	time.Sleep(5 * time.Second)
		...
		// 	By("Verifying app gets reconciled after label is added")
		...
		// 		Should(Not(Equal(v1beta2.ApplicationStateNew)))
