...
func NewStartCommand() *cobra.Command {
	...
	command.Flags().StringVar(&tlsMinVersion, "tls-min-version", "VersionTLS12",
		"Minimum TLS version for the webhook and metrics servers. "+
			"Possible values: VersionTLS12, VersionTLS13")
	command.Flags().StringSliceVar(&tlsCipherSuites, "tls-cipher-suites", []string{},
...
func start() {
	...
	// Create the manager.
	tlsResult, err := operatortls.SetupTLS(tlsMinVersion, tlsCipherSuites, enableHTTP2)
	if err != nil {
		logger.Error(err, "Failed to set up TLS")
		os.Exit(1)
	}
	tlsOptions := tlsResult.TLSOpts
